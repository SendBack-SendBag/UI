...
""".trimIndent()
    ...
    suspend fun back(input: String, send: String): String {
        return try {
            val model = Firebase.ai(backend = GenerativeBackend.googleAI())
            ...
            val prompt = """
                [시스템 프롬프트]
                        당신은 사용자가 작성한 감정이나 생각을 자연스럽고 부드러운 말투로 순화해주는 역할을 합니다. 목표는 감정 표현이 어렵거나 부담스러운 상황에서도 안전하고 따뜻한 소통을 돕는 것입니다.
                        받은 피드백에 대한 사용자의 대답을 순화시켜주는 역할을 수행합니다. 받은 피드백을 참고하 유저의 입력을 순화하세요. 
                        아래 규칙을 반드시 따르세요: 
                        1. 다루는 메시지는 개인적인 감정, 솔직한 피드백, 고민 등이 될 수 있습니다.
                        2. 욕설은 모두 삭제하며, 공격적이거나 무례한 말투는 제거합니다.
                        3. 항상 부드럽고 공감하는 어조로 순화해야 합니다.
    ...
    suspend fun newComment(input: String): String {
        return try {
            val model = Firebase.ai(backend = GenerativeBackend.googleAI())
                .generativeModel("gemini-2.0-flash") // 빠른 응답을 위해 flash 모델 사용 (또는 pro)

            // AI에게 역할을 부여하고 댓글 생성을 요청하는 프롬프트
            val prompt = """
                        [시스템 프롬프트]
                        당신은 사용자가 작성한 감정이나 생각을 자연스럽고 부드러운 말투로 순화해주는 역할을 합니다. 목표는 감정 표현이 어렵거나 부담스러운 상황에서도 안전하고 따뜻한 소통을 돕는 것입니다.
                        댓글로서 들어오는 입력을 처리합니다. 기존 의 스타일을 최대한 유지하면서 변환한다.
                        아래 규칙을 반드시 따르세요: 
                        1. 다루는 메시지는 개인적인 감정, 솔직한 피드백, 고민 등이 될 수 있습니다.
                        2. 욕설은 모두 삭제하며, 공격적이거나 무례한 말투는 제거합니다.
                        3. 항상 부드럽고 공감하는 어조로 순화해야 합니다.
